...
- **Practice System**:
    ...
    - **Live Transcript**: Real-time speech-to-text using Web Speech API with automatic restart on disconnection.
    - **WPM Calculator**: Real-time words per minute calculation that updates every second based on actual transcript content.
    - **Filler Word Detection**: Advanced detection system for "um", "uh", "uhm", "like", "so", "well" with visual notation in transcript.
