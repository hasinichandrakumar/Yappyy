...
Make it highly personal, engaging, and tailored to these specific requirements. Replace placeholders with relevant examples, adjust the tone and language to match the audience, and incorporate the personal elements naturally into the speech structure.`;
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.text(`Category: ${selectedTemplate.category || 'Custom'}`, 20, 40);
      pdf.text(`Duration: ${selectedTemplate.duration || 'Variable'}`, 20, 46);
      pdf.text(`Difficulty: ${selectedTemplate.difficulty || 'Not specified'}`, 20, 52);
      
      // Add a line separator
      pdf.setDrawColor(200, 200, 200);
      pdf.line(20, 58, pdf.internal.pageSize.getWidth() - 20, 58);
      ...
      const fileName = `${safeTitle.replace(/\s+/g, '-').toLowerCase()}-yappyy.pdf`;
      pdf.save(fileName);

      toast({ title: 'PDF Downloaded Successfully!', description: 'Your template has been exported as PDF.' });
    } catch (error: any) {
